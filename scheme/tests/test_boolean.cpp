...
TEST_CASE_METHOD(SchemeTest, "NotFunction") {
    ...
    ExpectEq("(not 0)", "#f");
    ExpectEq("(not '())", "#f");
    ExpectEq("(boolean? (not #f))", "#t");
    ExpectEq("(boolean? (not 1))", "#t");
}
...
TEST_CASE_METHOD(SchemeTest, "AndSyntax") {
    ...
    ExpectEq("(boolean? (and #t #f #t))", "#t");
    ExpectEq("(boolean? (and #t #t '4))", "#f");
}
...
TEST_CASE_METHOD(SchemeTest, "AndOptimizesArgumentEvaluation") {
    ...
    ExpectEq("(and #t #f (1 2))", "#f");
    ExpectRuntimeError("(and #t #t (1 2))");
}
...
TEST_CASE_METHOD(SchemeTest, "OrSyntax") {
    ...
    ExpectEq("(or #f (< 2 1))", "#f");
    ExpectEq("(or #f 1)", "1");
    ExpectEq("(boolean? (or #f #f #f))", "#t");
    ExpectEq("(boolean? (or #f #f -15))", "#f");
}
...
TEST_CASE_METHOD(SchemeTest, "OrOptimizesArgumentEvaluation") {
    ...
    ExpectEq("(or #f #t (1 2))", "#t");
    ExpectRuntimeError("(or #f #f (1 2))");
}
