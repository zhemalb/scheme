...
TEST_CASE_METHOD(SchemeTest, "IntegerMaxMin") {
    ...
    ExpectEq("(max 1 -2 5 3 4)", "5");
    ExpectEq("(min 1 2 3 -4 5)", "-4");
}
...
TEST_CASE_METHOD(SchemeTest, "IntegerPredicateAfterExecution") {
    ExpectEq("(number? (- 2 3))", "#t");
    ExpectEq("(number? '(/ 2 -1))", "#f");
    ExpectEq("(number? '())", "#f");
}
