...
TEST_CASE_METHOD(SchemeTest, "SimpleLambda") {
    ExpectEq("((lambda () (+ 5 6 7)))", "18");
    ExpectEq("((lambda (x) (+ 1 x)) 5)", "6");
}

TEST_CASE_METHOD(SchemeTest, "MultiBody") {
    ExpectEq("((lambda (x) (set! x (- 0 x)) x) 15)", "-15");
    ExpectEq("((lambda (x y) (define z (+ x y)) (* x y z)) 5 -3)", "-30");
}
...
TEST_CASE_METHOD(SchemeTest, "DefineLambdaSugar") {
    ...
    ExpectNoError("(define (f) (define f 15) (+ f 17))");
    ExpectEq("(f)", "32");
    ExpectEq("(f)", "32");
}
